        ...
        <table id="plan-plan-list" class="table table-striped data-table">
            ...
            <tbody>
                ...
                <tr>
					...
					<?php if($deposit->transaction_id !='' && $deposit->status=='Upload'){?>
                    <td><?php echo"Confirmation Pending"; ?></td>                   
            <?php } elseif($deposit->status=='Reject'){?>
				<td><?php echo"Rejected, Upload Slip again"; ?></td> 
				<?php }else{?>
				<td>Update Transaction Id</td>
				<?php }?>
				<td>
				<?php if($deposit->status=='Reject' || $deposit->status=='New'){ ?>
				 <span class="btn btn-xs btn-primary" id="upload" data-transaction_id="{{ $deposit->transaction_id }}" data-id="{{ $deposit->id }}" data-toggle="modal" data-target="#myModal"><i class="fa fa-upload" aria-hidden="true"></i> Update</span>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <p>{!!Form::open()
                        ...
                        ->method('POST')            
                        ->action(guard_url('blockchain_proof'))!!}
                        {{ csrf_field() }}
...
<script>
	...
	$.ajax({
                    url: "{{guard_url('delete_blockchain_request')}}",
                    type: 'POST',
                    data: {'id' : id},
